...
class JSON:
    Path = os.path.join(System.TOOL_PATH, "settings.json")
...
class Settings(BaseShell):
    ...
    def do_set(self, value):
        ...
        elif d[0] not in self.Data["settings"]:
            print(f"\033[1;33mNot \033[1;31m{d[0]} \033[1;33min Settings")
        else:
            value = d[1]
            try:
                value = value if (not (value in ["true", "false"]) and self.isbool == True) else \
                    {"true": True, "false": False}[value]
            except:
                print("\033[1;33mValue not in \033[1;31m[\033[1;32mtrue\033[1;31m,\033[1;32mfalse\033[1;31m]")
            else:
    ...
    def complete_set(self, arg, *args):
        ...
        if Name in a:
            ...
            if not arg:
                ...
                return [x for x in a if x.startswith(arg)]
